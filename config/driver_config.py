...
def get_driver(local=None, test_name="Test"):
    """
    Appium driver oluşturur.
    
    Args:
        local: True ise local emulator, False ise BrowserStack kullanır.
               None ise USE_LOCAL env variable'dan okur (varsayılan: False)
        test_name: BrowserStack'te görünecek test adı
    ...
    return webdriver.Remote(
        f"https://{bs_user}:{bs_key}@hub.browserstack.com/wd/hub",
        options=options
    )
